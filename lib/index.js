...
export function apply(ctx, config) {
  ...
  // ---------------------------------------------------------------------------
  // Camera capture support (capture_image tool)
  // ---------------------------------------------------------------------------

  /** Spawn `python capture.py` and resolve { code, err } when the child exits. */
  function runCaptureScript(args, timeoutMs) {
    const scriptPath = pathResolve(fileURLToPath(new URL('../scripts/', import.meta.url)), 'capture.py')
    return new Promise((resolve) => {
      let child
      try {
        child = spawn(process.env.PYTHON || 'python', [scriptPath, ...args], { windowsHide: true })
      } catch (err) {
        resolve({ code: 127, err: '无法启动 python: ' + err.message })
        return
      }
      let err = ''
      child.stderr.on('data', (d) => { err += String(d) })
      const timer = setTimeout(() => {
        try { child.kill() } catch (err2) { /* ignore */ }
      }, timeoutMs)
      child.on('close', (code) => {
        clearTimeout(timer)
  ...
  /**
   * capture_image tool body: take one photo via the USB camera, then run the
   * exact same recognition pipeline as analyze_image on the captured frame.
   * Returns the evidence plus capture metadata (saved path / camera / time).
   */
  async function captureAndAnalyze(args, exec) {
    const cwd = sessionCwd(exec)
    const camera = typeof args.camera === 'number' ? args.camera : 0
    const flip = typeof args.flip === 'string' && /^(none|h|v|b)$/.test(args.flip) ? args.flip : 'none'
    const capDir = cwd !== undefined ? pathResolve(cwd, '.captures') : pathResolve(process.cwd(), '.captures')
    await mkdir(capDir, { recursive: true })
    const stamp = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14)
    const outPath = pathResolve(capDir, 'cap-' + stamp + '.jpg')

    const width = 1280
    const height = 720
    const result = await runCaptureScript(['--out', outPath, '--camera', String(camera), '--width', String(width), '--height', String(height), '--flip', flip], 20000)
    if (result.code !== 0) {
      const detail = (result.err || '').trim().slice(0, 300)
      throw new Error('拍照失败（exit ' + result.code + '）: ' + (detail || '未知错误') + '。请确认 USB 摄像头已连接且未被占用；如无摄像头，请改用 analyze_image 并传入图片文件路径。')
    }

    const evidence = await analyzeImage({ image: outPath, question: args.question }, exec)
    evidence.capture = {
      path: outPath,
      camera: camera,
      flip: flip,
      ...
      schema: {
        ...
        properties: {
          ...
          capture: {
            type: 'object',
            additionalProperties: false,
            description: 'capture_image 工具的拍照元信息（analyze_image 不返回）。',
            properties: {
              path: { type: 'string', description: '抓拍图片保存路径。' },
              camera: { type: 'number', description: '摄像头设备号。' },
              flip: { type: 'string', description: '软件翻转方向：none/h/v/b。' },
              capturedAt: { type: 'string', description: '拍摄时间（ISO）。' },
            },
          },
        },
      },
  ...
  // ---------------------------------------------------------------------------
  // capture_image tool — USB camera photo + recognition (visual loop)
  // ---------------------------------------------------------------------------
  const captureTool = defineTool({
    name: 'capture_image',
    description: '通过 USB 摄像头自动拍摄一张照片并立即识别其内容（结构化证据：summary/ocr/layout/semantics/visual/uncertainty）。调试场景下模型可主动调用：改参数/烧录后拍摄观察屏幕或板卡的实际显示效果，形成"改→拍→看→再改"的视觉闭环；也可用于拍摄文档、标签做 OCR。无摄像头时请改用 analyze_image 并传图片文件路径。',
    parameters: {
      question: { type: 'string', description: '关于画面的具体问题（可选，作为识别聚焦方向，如"屏幕显示什么错误"）。' },
      camera: { type: 'number', description: '摄像头设备号（可选，默认 0）。' },
      flip: { type: 'string', enum: ['none', 'h', 'v', 'b'], description: '软件翻转方向（可选，默认 none）：应对俯拍/安装角度导致的图像倒置或镜像。' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          ocr: {
            ...
            properties: {
              ...
              lines: { type: 'array', required: true, items: { type: 'object', additionalProperties: false, properties: { text: { type: 'string', required: true } } } },
            },
          },
          layout: {
            type: 'object',
      ...
      render: function (args, value) {
        ...
        if (v.capture !== null && typeof v.capture === 'object' && typeof v.capture.path === 'string') {
          parts.push('图片已存: ' + v.capture.path)
        }
        if (Array.isArray(v.uncertainty) && v.uncertainty.length > 0) {
          parts.push('不确定: ' + v.uncertainty.join('; ').slice(0, 300))
        }
    ...
    async execute(args, exec) {
      return await captureAndAnalyze(args, exec)
    },
  })
  ...
  if (systemPrompt !== undefined) {
    try {
      systemPrompt.section({
        ...
        order: 105,
        text: '本会话的主模型无法直接查看图片。当用户要求查看、识别、描述或分析一张图片（给出文件路径或链接，或提到某张图片/截图/图表）时，必须调用 analyze_image 工具获取图片的结构化证据（OCR 全文、版面、场景、实体、不确定项），然后基于证据回答用户，引用具体内容而非凭空猜测。当需要观察实体屏幕/板卡/文档（调试时改参数后查看显示效果、拍摄文档 OCR 等）时，调用 capture_image 工具自动拍照并识别。视觉后端可通过配置的 backend 字段快速切换（deepseek / ollama / xiaomi / custom）。',
      })
    } catch (err) {
